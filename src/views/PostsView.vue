...
<template>
  <div>
    ...
    <div v-if="error" class="glass-card p-8 text-center text-red-500">{{ error }}</div>

    <div v-if="!loading && !error && results.length > 0" class="space-y-5">
      <article
        v-for="post in results"
        :key="post.slug"
        class="rounded-2xl overflow-hidden flex transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg"
        style="background: var(--glass-bg); backdrop-filter: blur(22px) saturate(1.4); -webkit-backdrop-filter: blur(22px) saturate(1.4); border: 1px solid var(--glass-border); box-shadow: var(--glass-shadow);"
      >
        <RouterLink
          :to="{ name: 'post', params: { slug: post.slug } }"
          class="w-46 shrink-0 flex items-center justify-center text-white/35 text-2xl"
          style="background: linear-gradient(135deg, #2d8a4e, #4caf50); min-height: 160px;"
        >
          {{ post.category || '📄' }}
        </RouterLink>
        <div class="p-6 flex-1 flex flex-col justify-center">
          <RouterLink
            :to="{ name: 'post', params: { slug: post.slug } }"
            class="text-lg font-bold mb-1 hover:underline"
            style="color: var(--color-text);"
          >
            {{ post.title }}
          </RouterLink>
          <div class="flex gap-4 flex-wrap text-xs mb-2" style="color: var(--color-text-muted);">
            <span>📅 {{ post.date }}</span>
    ...
    <div v-if="!loading && !error && results.length === 0" class="glass-card p-12 text-center">
